...
def create_netscape_cookies(cookies_dict, filename):
    # فتح الملف للكتابة
    with open(filename, 'w') as f:
        ...
        for domain, cookie_list in cookies_dict.items():
            for cookie in cookie_list:
                ...
                expires = cookie.get('expires', None)  # يمكن إضافة قيمة انتهاء الصلاحية
                secure = cookie.get('secure', False)

                # إذا لم يتم تحديد تاريخ انتهاء الصلاحية، يمكن استخدام 0
                if expires is None:
                    expires = 0
                else:
                    # تحويل تاريخ انتهاء الصلاحية إلى تنسيق UNIX timestamp
                    expires = int((expires - datetime(1970, 1, 1)).total_seconds())
...
cookies_dict = {
    ".youtube.com": [
        ...
        {'name': 'cookie_name_2', 'value': 'cookie_value_2', 'expires': datetime(2025, 1, 1)}
    ],
    ".example.com": [
        {'name': 'cookie_name_3', 'value': 'cookie_value_3', 'path': '/somepath'}
    ]
}

create_netscape_cookies(cookies_dict, 'cookies.txt')
